...
function calcTotals(items=diaryToday()){return items.reduce((a,it)=>{const f=byId(it.foodId)||it.food;const q=(it.tipo==='receita'||it.tipo==='tbca')?1:((Number(it.grams)||100)/100);['kcal','proteina','carbo','gordura','fibra','calcio','ferro','vitc','retinol','zinco','colesterol'].forEach(k=>a[k]+=(num(f?.[k])*q));return a},{kcal:0,proteina:0,carbo:0,gordura:0,fibra:0,calcio:0,ferro:0,vitc:0,retinol:0,zinco:0,colesterol:0})}
...
function renderMeals(){const el=document.getElementById('mealList');if(!el)return;const d=diaryToday();el.innerHTML=NV.meals.map(m=>{const itens=d.filter(x=>x.meal===m);const totals=calcTotals(itens);const itemHtml=itens.map(x=>{const f=byId(x.foodId)||x.food||{};const q=(x.tipo==='receita'||x.tipo==='tbca')?1:((Number(x.grams)||100)/100);const nome=x.tipo==='receita'?'🍽️ '+(f.nome||x.nome||'Receita'):(x.tipo==='tbca'?'🥗 '+(f.nome||x.nome||'Alimento'):(f.nome||'Alimento'));const sub=(x.tipo==='receita'||x.tipo==='tbca')?`${fmt(num(f.kcal))} kcal • ${x.tipo==='receita'?'receita':'TBCA'} • ${fmt(num(f.proteina),1)}g proteína`:`${fmt(num(f.kcal)*q)} kcal • ${fmt(num(f.proteina)*q,1)}g proteína • ${fmt(num(f.carbo)*q,1)}g carbo • ${fmt(num(f.gordura)*q,1)}g gordura`;const actions=(x.tipo==='receita'||x.tipo==='tbca')?`<button class="danger-btn" onclick="removeDiary('${x.id}')">Remover</button>`:`<input type="number" min="1" value="${Number(x.grams)||100}" onchange="updateDiaryGrams('${x.id}',this.value)" title="gramas"><span>g</span><button class="danger-btn" onclick="removeDiary('${x.id}')">Remover</button>`;return `<div class="diary-item"><div class="diary-main"><b>${nome}</b><small>${sub}</small></div><div class="diary-actions">${actions}</div></div>`}).join('');return `<div class="meal meal-block"><div class="meal-head"><div><h3>${m}</h3><small>${itens.length} item(ns) • ${fmt(totals.kcal)} kcal • ${fmt(totals.proteina,1)}g proteína</small></div><div class="meal-head-actions"><button class="ghost" onclick="openAutoMeal('${m}')">✨ Montar refeição</button><button class="ghost" onclick="openAdd('${m}')">+ Adicionar</button></div></div>${itens.length?`<div class="diary-list">${itemHtml}</div>`:'<div class="empty meal-empty">Nenhum alimento lançado nesta refeição.</div>'}</div>`}).join('')}
